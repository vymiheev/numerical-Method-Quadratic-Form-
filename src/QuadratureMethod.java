...
public class QuadratureMethod {
    ...
    private void countCoreKAndFx() {
        for (double x = EntryPoint.LOWER_LIMIT, row = 0; x <= EntryPoint.UPPER_LIMIT; x += EntryPoint.STEP, row++) {
            fx[((int) row)] = 1;
            for (double t = EntryPoint.LOWER_LIMIT, col = 0; t <= EntryPoint.UPPER_LIMIT; t += EntryPoint.STEP, col++) {
